...
const setTiles = () => {
    tiles.forEach((tile) => {
      const tileValue = randomOrderArray[i++];
...
const displayTile = (e) => {
    const tile = e.target; // Using e.target instead of this
    ...
    const tileIcon = tile.getAttribute("icon");
...
const checkMatch = (tileIcons, tileIds, n) => {
    ...
    if (tileIcons[n] !== tileIcons[n + 1]) {
        ...
        console.log("match");
        const tile1 = document.getElementById(tileIds[n]);
        const tile2 = document.getElementById(tileIds[n + 1]);
        ...
        tile1.removeEventListener("click", displayTile);
        tile2.removeEventListener("click", displayTile);
    }
}
...
const calculateScore = () => {
    ...
    document.querySelector("#score").textContent = calculatedScore;
}
